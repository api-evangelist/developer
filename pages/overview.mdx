...
schema, properties, experiences, lifecycle), white papers, conversations, services, and the API
vocabulary — plus the legacy static JSON feeds that predate the API.
...
- **Building blocks** — 2,400+ across nine collections: `/guidance`, `/rules`, `/policies`,
  `/standards`, `/strategies`, `/schema`, `/properties`, `/experiences`, `/lifecycle`
- **Media** — `/conversations`, `/papers`, `/newsletters`
- **Services** — Kin Lane's first-party offerings and the third-party vendor registry (`/services?party=first|third`)
- **Vocabulary** — the words used to describe APIs (`/vocabulary`)
- **Feeds** — a machine-readable index of the static feeds (`/feeds`)
